...
parser.add_argument('--speaker_info', type=str, help='speaker info file for labeled example (adding male/female) label')
...
id2gender = {}
lines = [line.strip() for line in open(paras.speaker_info)]
for line in lines:
    if line[0] == ";":
        continue
    parts = line.split("|")
...
for s in tr_set:
    ...
    spk_ids = [x.split("/")[1].split("-")[0] for x in sorted_todo]
    sorted_labels = [id2gender[x] for x in spk_ids]
    # Dump label
